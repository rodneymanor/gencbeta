...
const sampleVideos = [
  {
    ...
    metadata: {
      ...
      description:
        "Learn the secrets to building a content strategy that actually works in 2024. From platform selection to audience engagement, we cover everything you need to know to grow your following and increase your reach.",
    ...
    components: {
      ...
      bridge:
        "But before we dive into the strategy, let me share the three biggest mistakes I see creators making every single day.",
      nugget:
        "The key is not just creating content, but creating content that your audience actually wants to consume and share. This means understanding their pain points, desires, and what motivates them to take action.",
  ...
  },
  {
    ...
    metadata: {
      ...
      description:
        "Stop wasting time on Instagram Reels that don't work! Here are 5 proven hacks that will actually boost your engagement and reach. These aren't just tips - they're strategies that I've tested and proven to work.",
    ...
    components: {
      ...
      nugget:
        "The algorithm doesn't care about your follower count - it cares about engagement velocity. The faster you get engagement, the more the algorithm pushes your content.",
  ...
  },
  {
    ...
    metadata: {
      ...
      description:
        "Confused about whether to focus on YouTube Shorts or TikTok? I analyzed 1000+ creators and their performance data to give you the definitive answer. The results might surprise you!",
    ...
    components: {
      ...
      bridge:
        "But before I reveal which platform is actually better for creators, let me share the data that changed my entire perspective.",
      nugget:
        "The platform you should focus on depends entirely on your content type and audience demographics. YouTube Shorts favors educational content, while TikTok excels at entertainment.",
    ...
    <div className="bg-background min-h-screen p-6">
      <div className="mx-auto max-w-4xl space-y-8">
        <div className="space-y-4 text-center">
          <h1 className="text-foreground text-3xl font-bold">Video Insights Modal Redesign Test</h1>
          <p className="text-muted-foreground mx-auto max-w-2xl">
            This page demonstrates the new redesigned video insights modal with Instagram Reels-style layout, featuring
            split-screen design, consolidated actions, and improved mobile responsiveness.
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <h3 className="text-foreground font-semibold">New Features</h3>
                <p className="text-muted-foreground text-sm">
                  • Creator metadata in top bar
                  <br />
                  • Key KPIs (views, likes, comments, shares, completion)
                  <br />
                  • 1/3 video, 2/3 content layout
                  <br />
                  • Hook remix functionality
                  <br />
                  • Collapsible insights sections
                  <br />• Navigation between videos
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="text-foreground font-semibold">Current Video</h3>
                <p className="text-muted-foreground text-sm">
                  • {currentVideo.title}
                  <br />• Platform: {currentVideo.platform}
                  <br />• Author: {currentVideo.metadata.author}
                  <br />• Views: {currentVideo.metrics.views.toLocaleString()}
                  <br />• Duration: {currentVideo.duration}s
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-4 pt-4">
              <Button variant="outline" onClick={handlePrevious} disabled={sampleVideos.length <= 1}>
                Previous Video
              </Button>
              <span className="text-muted-foreground text-sm">
                {currentVideoIndex + 1} of {sampleVideos.length}
              </span>
              <Button variant="outline" onClick={handleNext} disabled={sampleVideos.length <= 1}>
                Next Video
              </Button>
            </div>
          </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-3">
                <h4 className="text-foreground font-semibold">Top Bar Enhancements</h4>
              ...
              <div className="space-y-3">
                <h4 className="text-foreground font-semibold">Layout & Functionality</h4>
                <ul className="text-muted-foreground space-y-1 text-sm">
              ...
              {sampleVideos.map((video, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="bg-primary/20 flex h-8 w-8 items-center justify-center rounded-full text-xs font-medium">
                      {index + 1}
                    </div>
                    <div className="flex-1">
                      <div className="text-foreground font-medium">{video.title}</div>
                      <div className="text-muted-foreground text-xs">
                        {video.metadata.author} • {video.platform} • {video.metrics.views.toLocaleString()} views
                      </div>
                    </div>
                    {index === currentVideoIndex && <div className="text-primary text-xs font-medium">Current</div>}
                  </div>
                </div>
              ))}
            </div>
