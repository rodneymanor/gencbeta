...
import { NotesControls } from "../../notes/_components/notes-controls";
...
export default function NotesPage() {
  ...
  const handleTitleEdit = (noteId: number, newTitle: string) => {
    const updatedNotes = notes.map((note) =>
      note.id === noteId ? { ...note, title: newTitle, updatedAt: new Date().toISOString().split("T")[0] } : note,
    );
    setNotes(updatedNotes);
    setFilteredNotes(updatedNotes);
