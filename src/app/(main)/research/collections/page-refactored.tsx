    ...
    <span className={`group relative ${className}`}>
      {editing ? (
        ...
        <button
          ...
          className={`hover:bg-secondary/50 rounded px-1 py-0.5 transition-colors ${className}`}
          disabled={loading}
        >
...
function CollectionsPageContent() {
  ...
  // Convert videos to VideoGridVideo format
  const videoGridVideos: VideoGridVideo[] = useMemo(() => {
    return videos.map((video) => ({
      ...
      addedAt: (video as any).addedAt,
    }));
  }, [videos]);

  // Custom badge renderer
  const renderBadge = useCallback((video: VideoGridVideo) => {
