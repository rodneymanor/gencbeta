...
export function VideoProcessingStatus({ 
  ...
  if (isLoading) {
    return (
      <div className={`animate-pulse bg-secondary/20 rounded-lg p-4 ${className}`}>
  ...
  if (!job) {
    return (
      <Alert className={className}>
        ...
        <AlertDescription>
          Unable to load processing status. Please refresh the page.
        </AlertDescription>
      </Alert>
    );
  }
...
function StatusDisplay({ job, isRetrying, onRetry, className }: StatusDisplayProps) {
      ...
      <div className={`bg-card border rounded-lg p-4 space-y-3 ${className}`}>
        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex items-center gap-2 text-green-800 mb-2">
            ...
            <span className="font-medium text-sm">Video processed successfully!</span>
          </div>
          <div className="text-xs text-green-700 space-y-1">
            <div><strong>Title:</strong> {job.result.title}</div>
            <div><strong>Platform:</strong> {job.result.platform}</div>
            <div><strong>Duration:</strong> {job.result.duration}s</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`bg-card border rounded-lg p-4 space-y-3 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`p-1 rounded-full ${STATUS_COLORS[job.status]}`}>
            <StatusIcon className={`h-3 w-3 text-white ${isActive ? 'animate-spin' : ''}`} />
          </div>
          <span className="font-medium text-sm">{STATUS_LABELS[job.status]}</span>
