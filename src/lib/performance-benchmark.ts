...
export class PerformanceBenchmark {
   ...
   */
  static async runBenchmark(iterations: number = 5, apiKey: string): Promise<BenchmarkSummary> {
   ...
   * Test V1 generation
   */
  private static async testV1(testCase: (typeof this.TEST_CASES)[0], apiKey: string): Promise<BenchmarkResult> {
   ...
   * Test V2 generation
   */
  private static async testV2(testCase: (typeof this.TEST_CASES)[0], apiKey: string): Promise<BenchmarkResult> {
    const startTime = Date.now();
  ...
  private static validateComponents(data: any): boolean {
    const checkOption = (option: any) => {
      ...
      return !!(option.elements.hook && option.elements.bridge && option.elements.goldenNugget && option.elements.wta);
    };
  ...
  private static calculateSummary(results: BenchmarkResult[]): BenchmarkSummary["v1"] {
    ...
    return {
      ...
      median_response_time: responseTimes.length > 0 ? responseTimes[Math.floor(responseTimes.length / 2)] : 0,
      p95_response_time: responseTimes.length > 0 ? responseTimes[Math.floor(responseTimes.length * 0.95)] : 0,
      average_word_count: wordCounts.length > 0 ? wordCounts.reduce((a, b) => a + b, 0) / wordCounts.length : 0,
  ...
  private static calculateComparison(
    ...
    const performanceImprovement =
      v1.average_response_time > 0
        ? ((v1.average_response_time - v2.average_response_time) / v1.average_response_time) * 100
        : 0;

    const successRateImprovement =
      v1.success_rate > 0 ? ((v2.success_rate - v1.success_rate) / v1.success_rate) * 100 : 0;

    const qualityImprovement =
      v1.component_completion_rate > 0
        ? ((v2.component_completion_rate - v1.component_completion_rate) / v1.component_completion_rate) * 100
        : 0;
