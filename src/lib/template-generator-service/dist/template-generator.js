...
class TemplateGenerator {
  geminiClient;
  constructor(apiKey) {
    this.geminiClient = new gemini_client_1.GeminiClient(apiKey);
  }
  /**
   * Generate templates from marketing segments (direct input)
   */
  async generateTemplatesFromSegments(segments) {
   ...
   */
  async generateTemplatesFromTranscription(transcription) {
    const startTime = Date.now();
    try {
   ...
   */
  async batchGenerateTemplates(inputs) {
    const startTime = Date.now();
    try {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        try {
          ...
          if (i < inputs.length - 1) {
            await gemini_client_1.GeminiClient.delay(this.geminiClient.getRateLimitDelay());
          }
        } catch (error) {
   ...
   */
  async createTemplateFromComponent(componentText, componentType) {
    const startTime = Date.now();
    try {
      console.log(`[TemplateGenerator] Creating template for ${componentType}: ${componentText.substring(0, 100)}...`);
      const prompt = prompts_1.TEMPLATE_GENERATION_PROMPTS.createTemplateFromComponent(componentText, componentType);
      const response = await this.geminiClient.makeTextRequest(prompt);
      // Parse the JSON response
      const parsedResponse = gemini_client_1.GeminiClient.parseJsonResponse(response.text);
  ...
  async analyzeTranscription(transcription) {
    try {
      ...
      const prompt = prompts_1.TEMPLATE_GENERATION_PROMPTS.analyzeTranscription(transcription);
      const response = await this.geminiClient.makeTextRequest(prompt);
      // Parse the JSON response
      const parsedResponse = gemini_client_1.GeminiClient.parseJsonResponse(response.text);
   ...
   */
  validateMarketingSegments(segments) {
    return (
      segments &&
