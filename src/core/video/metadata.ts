...
function extractDuration(rawMetadata: any, platform: Platform): number | undefined {
  switch (platform) {
    ...
    case 'youtube':
      return rawMetadata?.contentDetails?.duration ? 
        parseDuration(rawMetadata.contentDetails.duration) : undefined;
    default:
      return undefined;
  }
}
...
function extractUploadDate(rawMetadata: any, platform: Platform): string | undefined {
  switch (platform) {
    ...
    case 'instagram':
      return rawMetadata?.taken_at_timestamp ? new Date(rawMetadata.taken_at_timestamp * 1000).toISOString() : undefined;
    case 'youtube':
